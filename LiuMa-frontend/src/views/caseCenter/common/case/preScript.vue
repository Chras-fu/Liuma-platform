...
<template>
    <div>
        <el-table :data="pres"  size="small" row-key="index" :expand-row-keys="expands" @expand-change="expandSelect">
            <el-table-column type="expand">
                <template slot-scope="props">
                    <!-- 前后置脚本代码编辑 -->
                    <div v-if="props.row.name === 'preScript'" style="margin-top: 10px;">
                        <el-row>
                            <el-col :span="22">
                                <p class="tip">
                                    <span>编辑脚本</span>
                    ...
                    <!-- 前后置SQL编辑 -->
                    <div v-if="props.row.name === 'preSql'" style="margin-top: 10px">
                        <el-row>
                            <el-col :span="22">
                                <p class="tip">
                                    <span>编辑SQL</span>
            ...
            <el-table-column label="名称" prop="name" width="240">
                <template slot-scope="scope">
                    {{scope.row.name==='preScript'?"前置脚本":"前置SQL"}}
                    <el-tooltip placement="bottom">
                        <div v-if="scope.row.name==='preScript'" slot="content">
                            获取关联参数/公共参数使用sys_get(name)函数，<br/>
                            保存关联参数使用sys_put(name, val)函数，<br/>
                            保存公共参数使用sys_put(name, val, true)
                        </div>
            ...
            <el-table-column label="操作" width="100px">
                <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.$index!==(pres.length-1)" size="mini" type="text" icon="el-icon-bottom" @click="down(scope.$index)"/>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        addScript(){
            this.pres.push({index:this.pres.length+1, edit: false, name:"preScript", value: null });
            this.expands.push(this.pres.length);
        },
        addSql(){
            this.pres.push({index:this.pres.length+1, edit: false, name:"preSql", value: {
                db: "",
                sqlType: "",
        ...
        resetIndex(){
            for(let i=0;i<this.pres.length;i++){
                this.pres[i].index = i+1;
            }
        },
        up(index){
            this.pres[index-1]=this.pres.splice(index,1,this.pres[index-1])[0];
            this.resetIndex();
        },
        down(index){
            this.pres[index]=this.pres.splice(index+1,1,this.pres[index])[0];
            this.resetIndex();
        },
        remove(index){
            this.pres.splice(index, 1);
            this.resetIndex();
        },
        deleteAll(){
            this.pres.splice(0, this.pres.length);
        },
        saveScript(row){
            this.expands.splice(this.expands.indexOf(row.index), 1);
        },
